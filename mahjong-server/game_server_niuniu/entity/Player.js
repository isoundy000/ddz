...
Player.prototype.settlement = async function(totalWin,roomInfo){
    ...
    if(roomInfo.type =="jinbi"){
        if(this.numOfGame == roomInfo.maxGames){
            gameService.saveGameRecord(this.userId, this.name, "niuniu", 0, this.allToTalwin,roomInfo.seatCount,this.roomId,this.numOfGame,1,roomInfo.clubId, (err, result) => {
                if (err) {
                    console.log(err);
                }
            })
        }else{
            gameService.saveGameRecord(this.userId, this.name, "niuniu", 0, totalWin,roomInfo.seatCount,this.roomId,this.numOfGame,0,roomInfo.clubId, (err, result) => {
                if (err) {
                    console.log(err);
                }
            })
        ...
        if(this.numOfGame==roomInfo.maxGames){
            gameService.saveGameJiFenRecord(this.userId, this.name, "niuniu", 0, this.allToTalwin,roomInfo.seatCount,this.roomId,this.numOfGame,1,roomInfo.clubId, (err, result) => {
                if (err) {
                    console.log(err);
                }
            })
        }else{
            gameService.saveGameJiFenRecord(this.userId, this.name, "niuniu", 0, totalWin,roomInfo.seatCount,this.roomId,this.numOfGame,0,roomInfo.clubId,(err, result) => {
                if (err) {
                    console.log(err);
                }
            })
