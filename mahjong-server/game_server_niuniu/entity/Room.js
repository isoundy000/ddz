...
function Room(roomId, roomConfig, createUser) {
    ...
    //金币结算还是积分结算
    this.type = roomConfig.type;
    //玩家点击是否同意的解散的结果列表
    this.isJiesanList = [];
    //最低进入分数
    this.minScoreLimit = this.diZhu*this.qiangZhuangBeiShu*25;
    //最高进入分数
     ...
     */
    this.countdown = 0;
    this.paixing = roomConfig.paixing
    /**
     * 房间的定时器
     * @type {null}
     */
    ...
    for(let i=0;i<this.seats.length;i++){
        ...
        if(player.state!=player.PLAY_STATE.FREE){
            ...
            try {
                ...
                agentService.someLevelRebate(player.userId, fee, 3, '牛牛')
