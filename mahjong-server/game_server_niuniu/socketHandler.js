...
let playerService = require("../common/service/playerService")
...
if(!roomInfo){
    socket.emit('system_error', { errcode: 500, errmsg: "房间ID有误",roomId:roomId });
    return;
}
let player1 = roomInfo.getPlayerById(userId);
if(player1){
    agin_flag = true;
}
...
if(currentPlayer.isRobot==1){
    ...
    if(socket.handshake.headers['x-forwarded-for'] != null){
        ...
        console.log("ipppppppppp1",ip)
    }else{
        ip = socket.handshake.address;
        console.log("ipppppppppp2",ip)
    }
}
...
for (var i = 0; i < roomInfo.seats.length; i++) {
    ...
    if(player.state!=player.PLAY_STATE.FREE){
        ...
        if(player.optState==3){
            playerInfo.hold = gameLogic.group(roomId,player.hold);
...
ret.data.seats.forEach(i => {
    console.log("i.userId",i.userid,typeof (i.userid))
...
exports.robot_login =async function(socket, data,room_config){
    ...
    for (var i = 0; i < roomInfo.seats.length; i++) {
        ...
        if(player.state!=player.PLAY_STATE.FREE){
            ...
            if(player.optState==3){
                playerInfo.hold = gameLogic.group(roomId,player.hold);
                playerInfo.pokerType = gameLogic.getPokerType(roomId,player.hold);;
            }
            playerInfo.betBeiShu = player.betBeiShu;
        }
        seats.push(playerInfo);
...
exports.ready =async function(socket, data) {
    ...
    if(!roomInfo){
        socket.emit('system_error', { errcode: 500, errmsg: "您已不再此房间" });
        return;
    }
    if(roomInfo.gameState!=roomInfo.GAME_STATE.READY){
    ...
    console.log("this.seats.length",(roomInfo.seats.length))
    console.log("preparedPlayerCount",preparedPlayerCount)
    //console.log('*******当前房间玩家数量*********：'+playerCount);
    //console.log('*******当前房间准备后的玩家数量*********：'+preparedPlayerCount);
    //全部准备了，可以直接开始游戏
    // let ready_timeOuut = readyTimeOut(roomInfo.roomId);
    // roomInfo.setTimer(ready_timeOuut,5000);
    //先判断是否所有人都已经准备了
    if(((playerCount==preparedPlayerCount)&&preparedPlayerCount >1) ||(roomInfo.countdown==0 &&roomInfo.numOfGame !==0 )&&preparedPlayerCount >1){
        //如果倒计时还没结束，但玩家都准备了
...
exports.jiesan = async function(socket,data){
    ...
    for(let i of roomInfo.seats){
        i.setTimer(jiesanTimeOut,30000)
    }
}
//是否同意解散房间
exports.isJiesan = function(socket,data){
    ...
    if(roomInfo.isJiesanList.length==roomInfo.seatCount-1){
        let js = true;
        for(let i of roomInfo.isJiesanList){
            if(i.is==0){
                js=false;
            }
        }
        ...
        if(js){
            ...
            userMgr.broacastByRoomId("gb_jiesan_result",{errcode:200,errmsg:"解散失败",res:0},roomInfo.roomId);
...
async function getCtrl_param(userId){
    return new Promise( (resolve,reject)=>{
         playerService.getUserParamByUserId(userId,function(err,result){
            if(err){
                console.log(err)
                return;
            }
            if(result){
                resolve(result.ctrl_param)
            }
        });
    })
...
exports.gameBegin =async function(roomId){
    ...
    let fangzhuId = roomInfo.createUser;
    let fangzhu = roomInfo.getPlayerById(fangzhuId);
    if(room_type === "fangzhu" &&fangzhu &&!fangzhu.fangzhukouGems){
        
        fangzhu.fangzhukouGems=true;
        let gems = roomInfo.maxGames;
        fangzhu.settlementGems(0-gems,roomInfo);
        userMgr.broacastByRoomId('fangzhu_gems', { gems:fangzhu.gems},roomId);
    }
    ...
    let types = gameLogic.toPokerType(roomId,pokers);
    console.log('********排序前*********');
    console.log(types);
    //新增发牌控制
    var sorted = gameLogic.sortPoker(pokers,roomId);
    types = gameLogic.toPokerType(roomId,sorted);
    console.log('********排序后*********');
    console.log(types);
    var playerPokers = [];
    let i = 0;
    for(let j=0; j<roomInfo.seats.length;j++){
        ...
        if(player.state==player.PLAY_STATE.READY){
            ...
            let ctrl_param = await getCtrl_param(player.userId);
            ctrl_param = JSON.parse(ctrl_param);
            //是机器人
            let holdPoker = null;
            if(player.isRobot==1){
                holdPoker = gameLogic.getPokerByRadio(sorted,roomInfo.robot_param);
                let temp = gameLogic.getPokerType(roomId,holdPoker);
                console.log('*******发牌给机器人shenglv*******:'+roomInfo.robot_param);
            }else{
                holdPoker = gameLogic.getPokerByRadio(sorted,ctrl_param.niuniu);
                let temp = gameLogic.getPokerType(roomId,holdPoker);
                console.log('*******发牌给普通玩家shenglv*******:'+roomInfo.player_param);
            }
            playerDate.hold = holdPoker;
            player.mopai(holdPoker);
            player.setState(player.PLAY_STATE.PLAYING);
            playerPokers.push(playerDate);
            i++;
        }
    }
    ...
    console.log('**********系统发牌********');
    // console.log(JSON.stringify(playerPokers));
    // console.log(playerPokers)
    userMgr.broacastByRoomId('gb_begin_fapai',playerPokers,roomId);
    //延迟一秒后通知抢庄,切设置抢庄倒计时
    //根据玩家的数量，计算发牌时间
...
exports.yaZhu =async function(socket,data){
    ...
    var player = roomInfo.getPlayerById(userId);
    //检测分数是否足够
    if(player.coins<roomInfo.diZhu*beiShu && roomInfo.type=="jinbi"){
    ...
    userMgr.broacastInRoom('gb_yazhu',{userId:userId,beishu:beiShu},userId,true);
    console.log("同方玩家数据")
    console.log({userId:userId,beishu:beiShu})
    //检测闲家是否都已经押注了
    var isAllYaZhu = roomInfo.isAllOpt(roomInfo.GAME_STATE.YA_ZHU);

    //console.log('**********是否都押注了**********：'+isAllYaZhu);
    console.log("亮牌",player)
    if(isAllYaZhu){
        //通知亮牌
        ...
        roomInfo.setTimer(timeoutFunc,20000);
 ...
 * 提示
 */
exports.tiShi =async function(socket,data){
    if(typeof data ==="string"){
        data = JSON.parse(data);
    }
    var userId = data.userId;
    ...
    var type = gameLogic.getPokerType(roomInfo.roomId,pokers);
    //把有牛的牌放一块
    var grouped = gameLogic.group(roomInfo.roomId,pokers);
    if(socket){
        socket.emit('tishi_result',{hold:grouped,pokerType:type});
    }
}
...
function settlement(roomId){
    ...
    var roomInfo = gameMgr.getRoomById(roomId);
    //计算输赢
try{gameMgr.settlement(roomId);}catch(e){console.log(e)}
    //广播结算结果
    var results = [];
    for(var i=0;i<roomInfo.seats.length;i++){
        var player = roomInfo.seats[i];
        ...
        if(player.state!=player.PLAY_STATE.FREE){
            ...
            var type = gameLogic.getPokerType(roomInfo.roomId,pokers);
            //把有牛的牌放一块
            var grouped = gameLogic.group(roomId,pokers);
            res.hold = grouped;
            res.pokerType = type;
            ...
            if(roomInfo.room_type==="AA"&&player.totalWin >0){
                player.settlementGems(0-1,roomInfo);

            }
            if(player.totalWin >0){
                //扣除房间抽水
                roomInfo.choushui(player.totalWin);
            }
            res.coins = parseInt(player.coins);
            res.gems = parseInt(player.gems);
            res.allToTalwin = parseInt(player.allToTalwin);
            results.push(res);
        }
    }
    results.push({numOfGame:roomInfo.numOfGame})
    console.log('**********结算结果**********');
    console.log(results);
...
function gameOver(roomId){
    ...
    setTimeout(function () {
        ...
        // if(roomInfo.numOfGame === roomInfo.maxGames){
        //     userMgr.broacastByRoomId('gb_begin_destroy',{errcode:0,errmsg:"准备摧毁房间"},roomId);
        //     // setTimeout(function(){
                
        //     //     for(let i of roomInfo.seats){
        //     //         console.log("开始踢出房间");
        //     //         userMgr.sendMsg(i.userId,'tichu',{userId:i.userId});
        //     //         var userSocket = userMgr.get(i.userId);
        //     //         var data = {};
        //     //         data.userId = i.userId;
        //     //         exports.exit(userSocket,data);
        //     //     }
...
exports.huanfang=async function(socket,data,config,room_config){
    ...
    if(!is_robot){is_robot=0}

    //检查参数合法性
    if (!userId|| !time || !coins) {
        socket.emit('system_error', { errcode: 500, errmsg: "参数错误" });
        return;
    }
    //检查传过来的userId是否有误
    ...
    //     return;
    // }
    if (socket.hf_countdown != null && socket.hf_countdown < 0) { //5秒才能广播一次信息
        userMgr.sendMsg(userId, 'hf_msg', { msg: '您换房频率太快.歇一会吧' });
        return;
    }
    socket.hf_countdown = -1;
    setTimeout(() => {
        socket.hf_countdown = 1;
    }, 5000)
    //选择房间
    ...
    for (let i of keys){
        if((roomList[i].getPlayerCount() < roomList[i].seatCount)&&i!==roomId){
    ...
    //返回房间信息
    var roomInfo = gameMgr.getRoomById(new_roomId);
    console.log("roomid${roomId}",new_roomId)
    //获取玩家的坐位索引值
    var currentPlayer = roomInfo.getPlayerById(userId);
    if(!currentPlayer){
        socket.emit('system_error', { errcode: 500, errmsg: "加入房间失败,请稍后重试" });
    ...
    rechargeService.getShopInfoByShopId(params.prop_id, (err, res) => {
        ...
        if (!res) {
            ...
            if (res.price == 0) {
                ...
                if(player.coins<res.price && roomInfo.type=="jinbi"){
...
exports.ctrlKanPai =async function(socket){
    ...
    for (var i = 0; i < roomInfo.seats.length; i++) {
        ...
        if (player.state == player.PLAY_STATE.PLAYING) {
            ...
            data.pokers = gameLogic.group(roomInfo.roomId,player.hold);
            data.pokerType = gameLogic.getPokerType(roomInfo.roomId,player.hold);
            results.push(data);
        }
    }
