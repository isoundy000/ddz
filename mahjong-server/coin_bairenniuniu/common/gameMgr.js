...
exports.createRoom = async function (data) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        // if (data.coins < data.diZhu * data.minScoreLimit) {
        //     resolve(2221);
        //     return;
        // }
...
exports.enterRoom = async function (data) {
    ...
    let joinRoom = async function (room, roomId, userInfo) {
        try {
            ...
            await commonService.updateAsyncN('t_rooms', {
                [`user_id`]: userInfo.userId,
                [`user_icon`]: "",
                [`user_name`]: crypto.toBase64(userInfo.name),
                [`user_coins`]: userInfo.coins,
                [`user_ctrl_param`]: userInfo.ctrlParam,
            }, "id", roomId);
...
exports.exitRoom = function (userId) {
    ...
    //从数据库中移除玩家
    gameService.updateUserExitRoomN(userId,roomId, (err, result) => {
        if (err) {
            console.error(err);
        }
    });
...
exports.settlement = function(roomId){
    ...
    //庄家手牌
    var bankerHold = roomInfo.pokerUser[1].poker;
    var bankerPokerType = gameLogic.getPokerType(bankerHold);

    let zoushi={};
    for(var i=2;i<=5;i++){
        var totalWin = 0;
        var poker = roomInfo.pokerUser[i].poker;

            //闲家牌的类型
            var PokerType = gameLogic.getPokerType(poker);

            // var xianjiaBetBeiShu = xianjia.betBeiShu;
            var isWin = gameLogic.compare(bankerHold,poker);

            //庄家赢
            if(isWin==1){
                totalWin = gameLogic.radio[bankerPokerType]*(roomInfo.pokerUser[i].bets)*roomInfo.beishu;
                //扣除房间抽水
                roomInfo.choushui(totalWin);
                banker.updateTotalWin(totalWin);
                banker.updateWinNum();
                banker.settlement(totalWin,roomInfo);
                console.log("users",roomInfo.pokerUser[i].users)
                for(let j of roomInfo.pokerUser[i].users){
                    console.log("users",roomInfo.pokerUser[i].users)
                    let player = roomInfo.getPlayerById(j);
                    if(!player){
                        continue;
                    }
                    // banker.updateTotalWin(0-gameLogic.radio[bankerPokerType]*player.bet);
                    if(player.coins<=gameLogic.radio[bankerPokerType]*player.bet*roomInfo.beishu){
                        player.updateTotalWin(0-player.coins);
                        player.settlement(0-player.coins,roomInfo);
                    }else{
                        player.updateTotalWin(0-(gameLogic.radio[bankerPokerType]*player.bet*roomInfo.beishu),roomInfo);
                        player.settlement(0-(gameLogic.radio[bankerPokerType]*player.bet*roomInfo.beishu),roomInfo);
                    }
                    
                }
                zoushi[i]=0;
            }else{
                totalWin =gameLogic.radio[PokerType]*(roomInfo.pokerUser[i].bets*roomInfo.beishu);
                if(banker.coins<=totalWin){
                        banker.updateTotalWin(0-banker.coins);
                        banker.settlement(0-banker.coins,roomInfo);
                    }else{
                        banker.updateTotalWin(0-totalWin,roomInfo);
                        banker.settlement(0-totalWin,roomInfo)
                    }
                ...
                for(let j of roomInfo.pokerUser[i].users){
                    ...
                    player.updateTotalWin(gameLogic.radio[bankerPokerType]*player.bet*roomInfo.beishu);
                    player.updateWinNum();
                    player.settlement(gameLogic.radio[bankerPokerType]*player.bet*roomInfo.beishu,roomInfo);
                }
            }
        
    }

    roomInfo.updateZoushi(zoushi);
    return zoushi;
}
