...
exports.settlement = function(roomId){
    ...
    for(var i=0;i<roomInfo.seats.length;i++){
        ...
        if(xianjia.state!=xianjia.PLAY_STATE.FREE&&xianjia.isBanker==0){
            ...
            //闲家牌的类型
            var xianjiaPokerType = gameLogic.getPokerType(xianjiaHold);

            var xianjiaBetBeiShu = xianjia.betBeiShu;
            var isWin = gameLogic.compare(bankerHold,xianjiaHold);
            //庄家赢
            if(isWin==1){
                totalWin = xianjiaBetBeiShu*bankerBetBeiShu*gameLogic.radio[bankerPokerType]*diZhu;

                if (xianjia.coins<totalWin && roomInfo.type =="jifen"){
                ...
                if (banker.coins<totalWin && roomInfo.type =="jifen"){
